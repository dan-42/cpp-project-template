#ifndef PRE_XML_FROM_XML_HPP
#define PRE_XML_FROM_XML_HPP

#include <pre/xml/detail/parser.hpp>
...
namespace pre { namespace xml {
   ...
   */
  template<class T>
  T from_xml(const std::string& serialized_xml) {
    pugi::xml_document doc;
    doc.load_string(serialized_xml.c_str());
    T object;
    detail::parser parser(doc.first_child());
    parser(object);
    return object;
  }
   ...
   */
  template<class T>
  T from_json(const pugi::xml_node& doc) {
    T object;
